...
namespace UniversalTypeConverter.Tests {
    ...
    partial class TypeConverter_Tests {
        ...
        [TestMethod]
        public void Convert_Float_To_String_Should_Use_Given_Format() {
            var converter = new TypeConverter();
            float value = 1234.56f;

            converter.ConvertTo<string>(value).Should().Be(value.ToString());

            converter.Options.FloatFormat = "N2";
            converter.ConvertTo<string>(value).Should().Be(value.ToString("N2"));
        }

        [TestMethod]
        public void Convert_Float_To_String_Should_Use_The_Given_Culture() {
            var converter = new TypeConverter();
            float value = 1234.56f;
